...
*/
select count(tab2.cus_gender) as NoOfCustomers, tab2.cus_gender from
...
as tab1 
group by tab1.cus_id) as tab2 
group by tab2.cus_gender;
...
select `order`.*, TAB3.pro_name from `order`
inner join
...
supplier_pricing 
on
supplier_pricing.pro_id = product.pro_id ) as TAB3
on
`order`.pricing_id = TAB3.pricing_id where `order`.cus_id=2;
